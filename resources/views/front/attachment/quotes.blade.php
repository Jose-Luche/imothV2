        ...
        <div class="row">
            @foreach ($covers as $cover)
                <div class="col-lg-3 col-sm-6 col-md-6">
                    <a href="{{ route('front.attachment.details',['applicationId'=>$cover->id,'id'=>$cover->id])}}">
                        <div class="single-features-card bg-color-2">
                            <div class="features-icon">
                                <img style="width: 120px" src="{{ $cover->company->logo == null ? asset('images/logo.png'): asset('uploads/'.$cover->company->logo) }}">
                            </div>
                            <h3>{{ $cover->company->name }}</h3>
                            <p>{{ $cover->type }} </p>
                            <p>Selected Duration:
                                <span style="float: right">
                                    @if($duration == 'three_month')
                                        3 Months
                                    @elseif($duration == 'six_month')
                                        6 Months
                           ...
                           <br>
                            ...
                            <span style="float: right">
                                 @if($duration == 'three_month')
                                    {!! number_format($cover->three_month,2) !!}
                                @elseif($duration == 'six_month')
                                    {!! number_format($cover->six_month,2) !!}
                                @elseif($duration == 'one_year')
                                    {!! number_format($cover->one_year,2) !!}
                                @endif
                            </span>
