...
class ThirdPartyController extends Controller
{
    ...
    public function covers($id)
    {
        ...
        $covers = ThirdParty::where('category', $applicationDetails->vehicleUse)->orderBy('rate', 'ASC')->paginate(10);
    ...
    public function submitApplication($applicationId, $id)
    {
        ...
        $finalAmount = Session::get('totalPremium') ?? 0;
        ...
        $type = "paynow";
        if ($type === 'paynow'){
            return redirect()->route('front.third.pay',$applicationId)->with('success','Request received.Pay now to complete request.');
        }
