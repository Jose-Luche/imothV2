...
class SeniorsMedicalController extends Controller
{
    ...
    {
        return view('front.seniors.index');
    }
    ...
    public function submitHealthDetails(Request $request)
    {
        ...
        return redirect()->route('front.seniors.bio');
    }


    public function userBio()
    {
        return view('front.seniors.bio');
    }
    ...
    public function submitBio(Request $request)
    {
        ...
        $create = SeniorsMedicalInsuranceApplication::create([
        ...
        return redirect()->route('front.seniors.covers',$create->id);
    }

    public function covers($id)
    {
        $covers = SeniorsMedical::orderBy('limit','ASC')->where('benefit_type', 'inpatient')->orderBy('companyId')->paginate(10);
        $applicationDetails = SeniorsMedicalInsuranceApplication::find($id);
        ...
        foreach($covers as $cover){
            ...
            if(SeniorsMedicalPrincipalPremium::where('limitId', $cover->id)->first()){
                ...
                /**Basic Premium Part**/
                $principalPremiumDetails = SeniorsMedicalPrincipalPremium::where('limitId', $cover->id)
                ...
                if($applicationDetails->spouseAge > 0){
                    $spousePremiumDetails = SeniorsMedicalSpousePremium::where('limitId', $cover->id)
                ...
                foreach(SeniorsMedical::where('benefit_type', 'outpatient')->orderBy('limit', 'asc')->get() as $op){
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <option value="">--Select Plan--</option>';
                        foreach(SeniorsMedical::where('benefit_type', 'dental')->orderBy('limit', 'asc')->get() as $dental){
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <option value="">--Select Plan--</option>';
                foreach(SeniorsMedical::where('benefit_type', 'optical')->orderBy('limit', 'asc')->get() as $optical){
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <option value="">--Select Plan--</option>';
                foreach(SeniorsMedical::where('benefit_type', 'maternity')->orderBy('limit', 'asc')->get() as $maternity){
        ...
        return view('front.seniors.covers',[
            'html' => $html,
            'covers'=>$coverDetails,
     ...
     {
         $application = SeniorsMedicalInsuranceApplication::where('id', $id)->first();
         $res = 'error';
         if($application->update(
             ...
             $outpatientPremium = 0;
             $specificCover = SeniorsMedical::where('companyId', $insurerId)->where('benefit_type', 'outpatient')->where('pp_pf', $pp_pf)->where('limit', $limit)->first();
             ...
             if($specificCover){
                 if($pp_pf == 'pf'){
                     ...
                     $outpatientPremiumInstance = SeniorsMedicalFamilyPremium::where('limitId', $specificCover->id)->where('fm_age_from', '<=', $maxAge)
                     ...
                     $prinicipalInstance = SeniorsMedicalPrincipalPremium::where('limitId', $specificCover->id)->where('age_from', '<=', $application->principalAge)
                     ...
                     $spouseInstance = SeniorsMedicalSpousePremium::where('limitId', $specificCover->id)->where('sp_age_from', '<=', $application->spouseAge)
    ...
    public function updateDentalCover($id, $activator, $limit,$insurerId=null)
    {
        ...
        if($application->update(
            ...
            $res = 0;
            $specificCover = SeniorsMedical::where('companyId', $insurerId)->where('benefit_type', 'dental')->where('pp_pf', 'pp')->where('limit', $limit)->first();
            /**The Above Query will give us the Specific LimitID: Using it we can get premium Amounts Charged**/
            if($specificCover){
                /**Our Premiums will be from the Health Principal and Spouse Premium Table**/
                $principalPremium = $spousePremium = $childrenPremium = 0;
    ...
    public function updateOpticalCover($id, $activator, $limit, $insurerId=null)
    {
        ...
        if($application->update(
            ...
            $res = 0;
            $specificCover = SeniorsMedical::where('companyId', $insurerId)->where('benefit_type', 'optical')->where('pp_pf', 'pp')->where('limit', $limit)->first();
            /**The Above Query will give us the Specific LimitID: Using it we can get premium Amounts Charged**/
            if($specificCover){
                /**Our Premiums will be from the Health Principal and Spouse Premium Table**/
                $principalPremium = $spousePremium = $childrenPremium = 0;
    ...
    public function updateMaternityCover($id, $activator, $limit,$insurerId=null)
    {
        ...
        if($application->update(
            ...
            $res = 0;
            $specificCover = SeniorsMedical::where('companyId', $insurerId)->where('benefit_type', 'maternity')->where('pp_pf', 'pp')->where('limit', $limit)->first();
            /**The Above Query will give us the Specific LimitID: Using it we can get premium Amounts Charged**/
            if($specificCover){
                /**Our Premiums will be from the Health Principal and Spouse Premium Table**/
    ...
    public function updateSelectedCover($id,$limitId,$inpatientBasicPremium,$currentSelectedOutpatient,$currentSelectedDental,$currentSelectedOptical,$currentSelectedMaternity){
        $applicationDetails = SeniorsMedicalInsuranceApplication::findOrfail($id);
        ...
        if($applicationDetails){
            ...
            //return redirect()->route('front.seniors.details',['applicationId' => $applicationDetails->id, 'id' => $insurerId])->with('success','Your Option has been Saved for you');
        }
        return 'error';
    }
    ...
    public function coverDetails($applicationId,$id)
    {
        ...
        $applicationDetails = SeniorsMedicalInsuranceApplication::findOrfail($applicationId);
        $cover = $details;
        $coverDetails = [];
        ...
        return view('front.seniors.details',[
    ...
    public function submitApplication(Request $request,$id)
    {
       ...
       $request->session()->put('quoteId', $id);
        ...
        $create = SeniorsMedicalInsuranceApplication::findOrFail($id);
...
//        sendSms($create->phone,$message);
        ...
        $type = "paynow";
        if ($type === 'paynow'){
            return redirect()->route('front.seniors.pay',$create->id)->with('success','Request received.Pay now to complete request.');
        }
